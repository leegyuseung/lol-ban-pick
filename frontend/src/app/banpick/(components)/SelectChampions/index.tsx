    ...
    <div className="flex flex-col gap-3 md:w-[508px] md:order-3">
      ...
      >
        {Object.entries(filteredChampions).map(([name, info], idx) => (
          <div
            className={`relative flex flex-col items-center ${info.status !== '' ? 'cursor-not-allowed' : 'cursor-pointer'} pointerhover:hover:opacity-50 ${info.status != '' || name === currentSelectedPick[0].name ? 'opacity-20' : ''}`}
            key={name}
          >
            <ImageComp
              onLoad={imgLoadF}
      ...
      <div className="relative flex justify-center">
        ...
        {headerSecond !== '' && (
          <div className="md:absolute">
            <Button
              text={'챔피언 선택'}
              className={`${currentSelectedPick[0].name === '' || headerSecond === '' ? 'cursor-not-allowed' : 'cursor-pointer'} h-8 px-8 text-mainText bg-mainGold font-medium text-xs rounded-sm pointerhover:hover:bg-opacity-65`}
